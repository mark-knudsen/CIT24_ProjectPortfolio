...
CREATE OR REPLACE FUNCTION get_customer_rating(arg_customer_ID int4, arg_title_ID VARCHAR)
  RETURNS TABLE("customer_ID" int4, "title_ID" VARCHAR, "rating" NUMERIC(3,1), "created_at" TIMESTAMP) AS 
$BODY$
  BEGIN
    RETURN QUERY 
      SELECT * FROM customer_rating 
      WHERE customer_ID = arg_customer_ID AND title_ID=arg_title_ID;
  END;
$BODY$
...
$BODY$
  BEGIN
    RETURN QUERY
      SELECT * FROM customer_rating 
      WHERE customer_ID=arg_customer_ID ORDER BY created_at;
  END;
$BODY$
  LANGUAGE plpgsql VOLATILE 
...
CREATE OR REPLACE FUNCTION structured_search_query(arg_user_id int4, arg_title_name text=NULL::text, arg_plot text=NULL::text, arg_character text=NULL::text, arg_person_name text=NULL::text)
  RETURNS TABLE(title_id varchar, primary_name text) AS 
$BODY$
  ...
  BEGIN
    ...
    RETURN QUERY 
      SELECT DISTINCT title.title_id, title.primary_title 
      FROM title NATURAL JOIN plot NATURAL JOIN principal_cast NATURAL JOIN person 
      WHERE
        primary_title ilike '%'|| arg_title_name || '%' 
        AND plot.plot ilike '%' || arg_plot || '%' 
        AND principal_cast.character_name ilike '%' || arg_character || '%'
        AND person.primary_name ilike '%' || arg_person_name || '%' 
        AND (principal_cast.category = 'actor' OR principal_cast.category = 'actress');
  END;
$BODY$
  LANGUAGE plpgsql VOLATILE
...
CREATE OR REPLACE FUNCTION structured_actor_search_query(arg_user_id int4, arg_title_name text=NULL::text, arg_plot text=NULL::text, arg_character text=NULL::text, arg_person_name text=NULL::text)
  RETURNS TABLE(primary_name varchar, actor_character text) AS 
$BODY$
  ...
  BEGIN
    ...
    RETURN QUERY 
      Select distinct person.primary_name, principal_cast.character_name 
      FROM title NATURAL JOIN plot NATURAL JOIN principal_cast NATURAL JOIN person 
      WHERE
        primary_title ilike '%'|| arg_title_name || '%' 
        AND plot.plot ilike '%' || arg_plot || '%' 
...
$$ 
  ...
  BEGIN
    ...
    LOOP
      ...
      UPDATE person 
      SET person_average_rating = trunc((rec.vote_count*rec.average_rating)/(rec.vote_count),1) 
...
$$
  BEGIN 
    RETURN QUERY 
      SELECT rta.person_id, rta.primary_name, p.person_average_rating 
      FROM related_title_actors as rta NATURAL JOIN person as p  
      WHERE primary_title ilike '%' || arg_movie_title || '%' 
      ORDER BY person_average_rating DESC;    
  END;
$$;
